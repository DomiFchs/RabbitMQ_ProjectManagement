...
public class CreateProjectSerializer : JsonConverter<CreateAProjectDto> {
    public override CreateAProjectDto? Read(ref Utf8JsonReader reader,
        Type typeToConvert,
        ...
        if (isManagementProject) {
            return JsonSerializer.Deserialize<CreateManagementProjectDto>(root.GetRawText(), serializerCfg);
        }
        ...
        return JsonSerializer.Deserialize<CreateRequestFundingProjectDto>(root.GetRawText(), serializerCfg);
    }

    public override void Write(Utf8JsonWriter writer, CreateAProjectDto value, JsonSerializerOptions options) {
